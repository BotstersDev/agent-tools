#!/usr/bin/env node
/**
 * do-agent — Capability-first CLI for agents to interact with external services.
 */
...
function usage(): never {
  console.error(`Usage: do-agent <provider> <action> [args...] [flags]

Discovery:
  do-agent providers                List available providers
  do-agent <provider> actions       List actions for a provider
  do-agent <provider> <action> --help  Show action details
...
function main() {
  ...
  if (!schema) {
    ...
    console.error(`Run 'do-agent providers' to see available providers.`);
    process.exit(1);
  }
  ...
  if (!action) {
    ...
    console.error(`Run 'do-agent ${schema.name} actions' to see available actions.`);
    process.exit(1);
  }
