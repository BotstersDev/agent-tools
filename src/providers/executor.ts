...
export async function execute(
  ...
  if (schema.name === 'github') {
    headers['User-Agent'] = 'do-agent/1.0';
  }
...
async function executeGit(
  ...
  if (opts.dryRun) {
    process.stdout.write(JSON.stringify({ command: 'agent-git', args: ['clone', repoUrl, dest] }, null, 2) + '\n');
    return;
  }

  if (opts.verbose) {
    process.stderr.write(`agent-git clone ${repoUrl} ${dest}\n`);
  }

  const { execFileSync } = await import('node:child_process');
  try {
    execFileSync('agent-git', ['clone', repoUrl, dest], { stdio: 'inherit' });
  } catch {
    process.exit(1);
  }
}
