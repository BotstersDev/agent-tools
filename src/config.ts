...
import type { BrokerConfig } from '@botstersdev/broker-client';
...
export function loadConfig(): BrokerConfig {
  ...
  const agent = json['agent'] as Record<string, unknown> | undefined;
  const broker = agent?.['broker'] as Record<string, unknown> | undefined;
  if (!broker) {
    throw new Error('No agent.broker section in ~/.openclaw/openclaw.json');
  }
