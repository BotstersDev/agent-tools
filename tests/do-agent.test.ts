...
describe('buildUrl', () => {
  ...
  it('should substitute multiple path params', () => {
    const url = buildUrl('https://api.github.com', '/repos/{owner}/{repo}/issues', { owner: 'BotstersDev', repo: 'my-repo' });
    assert.equal(url, 'https://api.github.com/repos/BotstersDev/my-repo/issues');
  });
...
describe('parseActionArgs', () => {
  ...
  it('should parse mixed positional and flag args', () => {
    ...
    const result = parseActionArgs(action, ['BotstersDev', 'repo'], { title: 'Bug', body: 'Details' });
    assert.equal(result.path['owner'], 'BotstersDev');
